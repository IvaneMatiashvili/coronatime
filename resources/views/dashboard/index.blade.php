<x-dashboard-layout title="{{ __('content.worldwide-statistic') }}" content="worldwide" username="{{ $user->name }}">
    ...
    <x-flex.row class="w-[80rem] h-[15.938rem] ml-[9.25rem] justify-between mt-[2.5rem]">
        <x-flex.col class="w-[24.5rem] h-[15.938rem] bg-blue-bg bg-cover rounded-3xl bg-no-repeat bg-fixed">
            <x-svg.new-cases-svg/>
            <p class="font-medium text-[1.25rem] text-dark-100 h-[4rem]">{{ __('content.new-cases') }}</p>
        ...
        <x-flex.col class="w-[24.5rem] h-[15.938rem] bg-green-bg bg-cover rounded-3xl bg-no-repeat bg-fixed">
            <x-svg.recovered-svg/>
            <p class="font-medium text-[1.25rem] text-dark-100 h-[4rem]">{{ __('content.recovered') }}</p>
        ...
        <x-flex.col class="w-[24.5rem] h-[15.938rem] bg-yellow-bg bg-cover rounded-3xl bg-no-repeat bg-fixed">
            <x-svg.death-svg/>
            <p class="font-medium text-[1.25rem] text-dark-100 h-[4rem]">{{ __('content.deaths') }}</p>
